...
if not infile_or_ip or not sc_mode or sc_help:
    ...
    print(
        "   -c <channel>    : the first characters of a channel name (DAHDI, SIP, IAX)"
    )
    ...
    print("                     format like 2009-11-27 (default is current day)")
    print(
        "   -i              : outputs a PNG graph under simultcalls_%Y%m%d_%H%M%S.png"
    )
    print(
        "   -p              : outputs a PostScript graph under simultcalls_%Y%m%d_%H%M%S.ps"
    )
...
if sc_mode == "ip":
    ...
    w = urllib.request.urlopen(
        "https://%s/service/ipbx/json.php/%s/call_management/cdr/?act=search&dbeg=%s&dend=%s"
    ...
    if wr:
        ...
        print(
            " - did you give some meaningful begin and end dates ? (%s -> %s)"
            % (sc_begin, sc_end)
        )
    ...
    if infile_or_ip.startswith("http:"):
        f = urllib.request.urlopen(infile_or_ip)
...
for st in mystats:
    ...
    deltatime_s = st[1]
    for ttime_s in range(time_s, time_s + deltatime_s + 1):
...
t3 = time.time()
print(
    "... (%.1f seconds spent, %d bins filled)\n"
